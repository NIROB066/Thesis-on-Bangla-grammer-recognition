#Input Read From Excel File
all_input_sentences = []
import xlrd
import openpyxl
#File location
loc = ("C:\\Users\\riyar\\OneDrive\\Desktop\\Thesis APP\\input_documentary.xlsx")
#File Location
wb = xlrd.open_workbook(loc)
sheet = wb.sheet_by_index(0)
sheet.cell_value(0, 0)
for i in range(1, sheet.nrows):
    all_input_sentences.append(sheet.cell_value(i, 0))
#Input Read From Excel File
...
# APP
#Now we will work with in the excel file
wb = openpyxl.load_workbook(loc)
ws = wb["Sheet1"]
length_all_input_sentence = len(all_input_sentences)
for sentences in range(length_all_input_sentence):
    xxx1 = all_input_sentences[sentences]
    print("Give the input language...")
    input_sentence = xxx1.split(" ")
    # space handling
    alt_input_sentence = []
    for space in range(len(input_sentence)):
        if input_sentence[space] != "" and input_sentence[space] != "|" and input_sentence[space] != "?" and \
                input_sentence[space] != "।" and input_sentence[space] != "." and input_sentence[space] != "," and \
    ...
    wcell1 = ws.cell(sentences+2, 5)
    wcell1.value = output_sentence

    wcell1 = ws.cell(sentences+2, 4)
    wcell1.value = output_status
wb.save(loc)
    # Writing To excel file
